...
A hands-on walkthrough of running a metrics + logs observability stack on a local **kind** cluster. A single Go HTTP server in [`app/`](app) (`obs`) acts as the workload: it exposes Prometheus metrics on `/metrics` and emits structured JSON logs on stdout. Two operator-driven stacks observe it independently:
